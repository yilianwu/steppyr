...
public:
    ...
    */
    ...
    virtual void set_target_speed(double speed);

    // Acceleration
    double current_acceleration() const;
    double target_acceleration() const;
    virtual void set_target_acceleration(double acceleration);

    // Step
    long current_steps() const;
    virtual void set_current_steps(long position);
    virtual void set_target_steps(long absolute_steps);

    long microsteps() const;
    virtual void set_microsteps(long microsteps);
...
protected:
    ...
    double calc_speed_from_rpm(double rpm) const;
};

double calc_step_interval_us(double speed);
