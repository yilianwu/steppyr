...
PYBIND11_MODULE(steppyr, m) {
    ...
    py::class_<StepperController>(m, "StepperController")
        ...
        .def_property_readonly("current_speed", &StepperController::current_speed)
        .def_property_readonly("target_speed", &StepperController::target_speed)
        .def("set_target_speed", &StepperController::set_target_speed)
        .def_property_readonly("current_acceleration", &StepperController::current_acceleration)
        .def_property_readonly("target_acceleration", &StepperController::target_acceleration)
        .def("set_target_acceleration", &StepperController::set_target_acceleration)
        .def_property_readonly("current_steps", &StepperController::current_steps)
        .def("set_current_steps", &StepperController::set_current_steps)
        .def("set_target_steps", &StepperController::set_target_steps)
        .def_property_readonly("full_steps_per_rev", &StepperController::full_steps_per_rev)
        .def("set_full_steps_per_rev", &StepperController::set_full_steps_per_rev)
        .def("set_target_rpm", &StepperController::set_target_rpm)
        .def_property_readonly("steps_to_go", &StepperController::steps_to_go)
        .def_property_readonly("is_moving", &StepperController::is_moving)
        .def_property_readonly("direction", &StepperController::direction)
    ;
